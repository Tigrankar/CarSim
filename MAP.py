...
brain = Dqn(5,3,0.9) #brain = Dqn(5,3,0.9)
...
class Game(Widget):
    ...
    def update(self, dt):
        ...
        if sand[int(self.car.x),int(self.car.y)] > 0:
            ...
            last_reward = -1  #last_reward=-1
        else:
            self.car.velocity = Vector(6, 0).rotate(self.car.angle)
            last_reward = -0.2
            if distance < last_distance:
                last_reward = 0.1    #last_reward =0.2
...
class CarApp(App):
    ...
    def save(self, obj):
        ...
        root.withdraw()
        #save brain
        pth_file_path = asksaveasfilename(title='Save PTH file', filetypes = (("PTH files","*.pth"),("all files","*.*")))
        if pth_file_path==():
            pth_file_path=""
        if pth_file_path=="":
    ...
    def load(self, obj):
        ...
        root.withdraw()
        #load brain
        pth_file_path = askopenfilename(title='Open PTH file', filetypes = (("PTH files","*.pth"),("all files","*.*")))
        if pth_file_path==():
            pth_file_path=""
        if pth_file_path=="":
    ...
    def mapsave(self, obj):      
        ...
        #save png file
        png_file_path = asksaveasfilename(title='Save PNG file', filetypes = (("PNG files","*.png"),("all files","*.*")))
        if png_file_path==():
            png_file_path=""
        if png_file_path=="":
        ...
        #save npy file
        npy_file_path = asksaveasfilename(title='Save NPY file', filetypes = (("NPY files","*.npy"),("all files","*.*"))) 
        if npy_file_path==():
            npy_file_path=""
        if npy_file_path=="":
    ...
    def mapload(self, obj):
        ...
        #open png file
        png_file_path = askopenfilename(title='Open PNG file', filetypes = (("PNG files","*.png"),("all files","*.*")))
        if png_file_path==():
            png_file_path=""
        ...
        #open npy file
        npy_file_path = askopenfilename(title='Open NPY file', filetypes = (("NPY files","*.npy"),("all files","*.*")))
        if npy_file_path==():
           npy_file_path=""
